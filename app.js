...
function renderRk(){const sel=$("selRkEv").value,ap=catches.filter(c=>c.status==="approved"&&(!sel||c.ev===sel));const pl={};ap.forEach(c=>{if(!pl[c.name])pl[c.name]={name:c.name,fish:[]};pl[c.name].fish.push(c)});const rk=Object.values(pl).map(p=>{p.fish.sort((a,b)=>b.size-a.size);const top3=p.fish.slice(0,3);p.total=top3.reduce((s,f)=>s+f.size,0);p.cnt=p.fish.length;return p}).sort((a,b)=>b.total-a.total);let lh="";rk.forEach((p,i)=>{const rc=i===0?"rk-1":i===1?"rk-2":i===2?"rk-3":"";const top3=p.fish.slice(0,3);lh+='<div class="rr" data-i="'+i+'"><div class="rk '+rc+'">'+(i+1)+'</div><div class="rn">'+p.name+'</div><div class="rt">'+p.total.toFixed(1)+'</div><div class="rc">'+p.cnt+L.fish_count+'</div></div><div class="rd" id="rd'+i+'">';top3.forEach((f,j)=>{lh+='<div class="rd-f"><span>★'+(j+1)+' '+f.fish+' '+f.size+'cm</span><span>'+f.time+'</span></div>'});if(p.fish.length>3){lh+='<div class="rd-f" style="color:#999"><span>--- other '+(p.fish.length-3)+' ---</span><span></span></div>'}lh+='</div>'});$("rkList").innerHTML=lh||'<div style="text-align:center;padding:30px;color:#999;font-size:13px">'+L.noData+"</div>";document.querySelectorAll(".rr").forEach(r=>r.addEventListener("click",function(){$("rd"+this.getAttribute("data-i")).classList.toggle("on")}))}
