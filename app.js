...
function renderRk(){
const sel=$("selRkEv").value;
const ap=catches.filter(c=>c.status==="approved"&&(!sel||c.ev===sel));
const pl={};
ap.forEach(c=>{if(!pl[c.name])pl[c.name]={name:c.name,fish:[]};pl[c.name].fish.push(c)});
const rk=Object.values(pl).map(p=>{
p.fish.sort((a,b)=>b.size-a.size);
const top3=p.fish.slice(0,3);
p.total=top3.reduce((s,f)=>s+f.size,0);
p.cnt=p.fish.length;
return p;
...
lh+='<div class="rr" data-i="'+i+'"><div class="rk '+rc+'">'+(i+1)+'</div><div class="rn">'+p.name+'</div><div class="rt">'+p.total.toFixed(1)+'</div><div class="rc">'+p.cnt+L.fish_count+'</div></div>';
lh+='<div class="rd" id="rd'+i+'">';
p.fish.forEach((f,j)=>{
if(j<3){lh+='<div class="rd-f"><span>★'+(j+1)+' '+f.fish+' '+f.size+'cm</span><span>'+f.time+'</span></div>';}
else{lh+='<div class="rd-f" style="color:#bbb"><span>　　'+f.fish+' '+f.size+'cm '+L.otherFish+'</span><span>'+f.time+'</span></div>';}
});
lh+='</div>';
});
$("rkList").innerHTML=lh||'<div style="text-align:center;padding:30px;color:#999;font-size:13px">'+L.noData+"</div>";
document.querySelectorAll(".rr").forEach(r=>r.addEventListener("click",function(){$("rd"+this.getAttribute("data-i")).classList.toggle("on")}));
}

$("bCSV").addEventListener("click",()=>{const sel=$("selRkEv").value,ap=catches.filter(c=>c.status==="approved"&&(!sel||c.ev===sel));const pl={};ap.forEach(c=>{if(!pl[c.name])pl[c.name]={name:c.name,fish:[]};pl[c.name].fish.push(c)});const rk=Object.values(pl).map(p=>{p.fish.sort((a,b)=>b.size-a.size);const top3=p.fish.slice(0,3);p.total=top3.reduce((s,f)=>s+f.size,0);p.best=p.fish[0]?p.fish[0].size:0;p.cnt=p.fish.length;return p}).sort((a,b)=>b.total-a.total);const d=[["Rank","Name","Top3 Total(cm)","Best(cm)","Count"]];rk.forEach((p,i)=>d.push([i+1,p.name,p.total.toFixed(1),p.best.toFixed(1),p.cnt]));const csv="\uFEFF"+d.map(r=>r.join(",")).join("\n"),bl=new Blob([csv],{type:"text/csv"}),a=document.createElement("a");a.href=URL.createObjectURL(bl);a.download="ranking.csv";a.click()});
